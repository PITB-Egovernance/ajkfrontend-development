        ...
        $(document).ready(function() {
            // Auto-generate Diary/Outward No if not already set (e.g., from old input)
            if (!$('#diary_outward_no').val()) {
                $('#diary_outward_no').val('AUTO-' + Math.floor(Math.random() * 1000000));
            }

            // Form submit handler (optional client-side, but backend handles validation now)
            $('#correspondenceForm').on('submit', function(e) {
                // Client-side can be minimal since backend validates
            });

            // Toast function (for client-side if needed, but success/error handled by session now)
            // function showToast(type, message) {
            //     const toastId = type === 'success' ? 'successToast' : 'errorToast';
            //     const $toastBody = $('#' + type + 'ToastBody');
