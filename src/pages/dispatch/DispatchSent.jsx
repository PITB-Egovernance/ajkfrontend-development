...
const DispatchSent = () => {
  ...
  const fetchForms = async (pageIndex = 0, pageSize = 10) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/dispatch/dispatched-forms`,
  ...
  const fetchFormDetails = async (id) => {
    try {
      const response = await fetch(`${API_BASE}/dispatch/dispatched-forms/${id}`, {
        headers: fetchHeaders,
      });
        ...
        >
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${getPriorityClass(selectedForm?.priority)}`}>
                {selectedForm?.priority || 'N/A'}
              </span>
            </Grid>
            <Grid item xs={12} sm={6}>
