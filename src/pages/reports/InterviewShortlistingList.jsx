...
const InterviewShortlistingList = () => {
  ...
  const fetchReport = useCallback(async () => {
    setSearching(true);
    try {
      const result = await ReportsApi.getInterviewShortlist({
      ...
      toast.error(err?.message || 'Failed to load interview shortlisting list');
      setRows([]);
      setTotalCount(0);
    } finally {
      setSearching(false);
  ...
  const columns = [
    ...
    { field: 'interviewDate', headerName: 'Interview Date', width: 140,
      renderCell: (p) => p.value || <span className="text-slate-400 text-xs">Not scheduled yet</span> },
