...
const mapRow = (row) => ({
  ...
  totalPossibleMarks: row.total_possible_marks,
...
const MeritListReport = () => {
  ...
  const fetchReport = useCallback(async () => {
    setSearching(true);
    try {
      const result = await ReportsApi.getMeritList({
      ...
      toast.error(err?.message || 'Failed to load merit list report');
      setRows([]);
      setTotalCount(0);
    } finally {
      setSearching(false);
  ...
  const columns = [
    ...
    { field: 'totalMarks', headerName: 'Total Marks', width: 120, type: 'number',
      renderCell: (p) => `${p.value}/${p.row.totalPossibleMarks}` },
    { field: 'percentage', headerName: 'Percentage', width: 120, renderCell: (p) => `${p.value}%` },
    {
      field: 'meritStatus', headerName: 'Merit Status', width: 160, sortable: false,
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <div className="mx-auto bg-white rounded-xl shadow-sm p-6" style={{ minWidth: '-webkit-fill-available' }}>
        <ReportPageHeader
          icon={Trophy}
          title="Merit List (Category Wise)"
          subtitle="Category-wise merit ranking by open merit, gender, and district quotas"
          breadcrumbs={[
            { label: 'Reporting & Analytics' },
            { label: 'Marks & Result Reports' },
            { label: 'Merit List (Category Wise)' },
          ]}
        />
        ...
        <ReportTable
          ...
          emptyTitle="No merit list records found"
