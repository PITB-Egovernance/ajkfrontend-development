...
const TITLE_PRESETS = ["Secretary", "Member"];
...
const SecretaryOfficialsManagement = () => {
  ...
  const filterConfig = [
    ...
    },
    {
      ...
      label: "Title",
      type: "text",
      placeholder: "Filter by title (e.g. Secretary, Member)",
    },
    {
      name: "status",
      label: "Status",
  ...
  /* ===============================
     FETCH SECRETARY AND OFFICIALS
  =============================== */
  const fetchOfficials = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/settings/secretary-officials`, {
        headers: authHeaders(),
      });
      ...
      if (res.ok || result.success || result.status === 200) {
        ...
        toast.error(result.message || "Failed to load secretary and officials");
        setRows([]);
      }
    } catch {
      toast.error("Failed to load secretary and officials");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await confirmDelete({
      title: "Delete Secretary and Officials?",
    ...
    try {
      const res = await fetch(
        `${API_BASE}/settings/secretary-officials/${targetMember.hash_id}/delete`,
  ...
  const handleToggleStatus = async (row, currentStatus) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/settings/secretary-officials/${row.hash_id}/update`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isUpdate
        ? `${API_BASE}/settings/secretary-officials/${editingMember.hash_id}/update`
        : `${API_BASE}/settings/secretary-officials/store`;
  ...
  if (loading && rows.length === 0) {
    return <InlineLoader text="Loading secretary and officials..." variant="ring" size="lg" />;
  }
    ...
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Manage Secretary and Officials listings shown across the system.
            </p>
          </div>

          <Button onClick={handleOpenAdd}>
            <Plus className="w-4 h-4 mr-2" />
            Add Secretary and Officials
          </Button>
        </div>
        ...
        <AdvancedFilter
          ...
          title="Filter Secretary and Officials"
        />
          ...
          {filteredRows.length > 0 ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-12 text-center shadow-sm flex flex-col items-center justify-center my-4">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-1">
                No Secretary and Officials Found
              </h3>
        ...
        >
          <DialogTitle className="bg-gradient-to-r from-emerald-800 to-emerald-950 text-white flex items-center justify-between py-4 px-6">
            <span className="flex items-center gap-2 text-lg font-bold">
              ...
              {editingMember ? "Edit Secretary and Officials" : "Add Secretary and Officials"}
            </span>
            <IconButton
              ...
              <div className="flex flex-col items-center justify-center p-4 bg-slate-50 border border-dashed border-slate-300 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="hidden"
                    id="secretary-official-image-upload"
                  />
                  <label
                    htmlFor="secretary-official-image-upload"
