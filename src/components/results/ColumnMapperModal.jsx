...
const ColumnMapperModal = ({
  ...
  return (
    ...
    >
      {/* Dialog Header */}
      <DialogTitle className="flex justify-between items-center pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-emerald-50 text-emerald-600 rounded-2xl">
            <FileSpreadsheet size={24} />
          </div>
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">Map CSV Columns</h2>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Flexible Column Ingestion</p>
          </div>
        </div>
        <IconButton onClick={onClose} className="text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-full">
          <X size={20} />
        </IconButton>
      </DialogTitle>
      ...
      <DialogContent className="py-6 space-y-6 max-h-[60vh] overflow-y-auto">
        ...
        {sparkleActive && (
          <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl p-4 flex items-center gap-3 animate-pulse">
            <Sparkles className="text-emerald-600 shrink-0" size={18} />
            <p className="text-xs font-bold text-emerald-800">
              Smart Suggestion: Auto-mapped matching headers using Fuzzy Logic!
            </p>
          </div>
        )}
        ...
        {/* Validation Errors */}
        {validationError && (
          <div className="bg-red-50 border border-red-200 text-red-700 rounded-2xl p-4 flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
            <AlertCircle className="shrink-0" size={18} />
            <p className="text-xs font-black">{validationError}</p>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 bg-white p-5 border border-slate-100 rounded-3xl shadow-sm">
            {subjects.length === 0 ? (
              <div className="space-y-3">
                ...
                {csvHeaders
                  .filter((h) => h !== mappings.identifier_column)
                  .map((colName) => {
                      ...
                      <div
                        ...
                        className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer select-none ${
                          isChecked
                            ...
                            : 'border-slate-100 bg-slate-50/30 text-slate-600 hover:bg-slate-50 hover:border-slate-200'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <input
                          ...
                          />
                          <span className="font-extrabold text-sm tracking-tight">{colName}</span>
                        </div>
                        <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full transition-all ${
                          isChecked
              ...
              subjects.map((subject) => {
                ...
                return (
                  <div 
                    key={subject.id} 
                    className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 hover:bg-slate-50/50 rounded-2xl border border-slate-50 transition-colors"
                  >
                    <div>
                      <span className="font-extrabold text-slate-800 text-sm">{subject.subject_name}</span>
                      <span className="ml-2 text-xs font-bold text-slate-400">(Max Marks: {subject.max_marks})</span>
                    </div>
      ...
      {/* Dialog Actions */}
      <DialogActions className="p-6 border-t border-slate-100 justify-between">
        <Button variant="outline" onClick={onClose} className="rounded-xl px-6 font-bold text-xs uppercase tracking-widest text-slate-500">
          Cancel
        </Button>
        <Button 
          variant="primary" 
          onClick={handleConfirm}
          className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-8 font-black text-xs uppercase tracking-widest shadow-md hover:shadow-lg transition-all"
        >
          Confirm & Validate
        </Button>
      </DialogActions>
